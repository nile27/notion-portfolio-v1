...
*   **해결**: 루트에 `loading.tsx`를 배치하거나 데이터를 긁어오는 컴포넌트를 React `<Suspense>`로 래핑하여 **HTML 스트리밍(Streaming)** 기법 도입.
*   **성과**: 데이터 요청의 지연 여부와 상관없이 사용자가 접속 즉시 공통 레이아웃과 스켈레톤/스피너 등의 로딩 표시기를 보게 되어 체감 대기 속도가 획득되고 UX 안정성이 대폭 증대됨.
<br />
<br />
